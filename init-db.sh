...
service mariadb start || mysqld_safe --user=mysql &

# Esperar a que MySQL esté listo via socket
...
# Conectar via socket unix (sin contraseña - auth_socket de Debian)
...
if [ -f /var/www/html/base-datos/tablas/sistema_agencia.sql ]; then
    ...
    mysql --socket=/var/run/mysqld/mysqld.sock -u root sistema_agencia < /var/www/html/base-datos/tablas/sistema_agencia.sql
...
if [ -f /var/www/html/base-datos/permisos/grants_usuarios_basicos.sql ]; then
    ...
    mysql --socket=/var/run/mysqld/mysqld.sock -u root sistema_agencia < /var/www/html/base-datos/permisos/grants_usuarios_basicos.sql
